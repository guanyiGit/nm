...
Page({
  ...
  searchStrayDog(e) {
    var that = this;
    var query = that.data.query ? that.data.query:{};
    // if (that.data.startDate == "开始时间"){
    if (that.data.startDate == wx.T.get("kaishishijian")){
      query.startDate = ""
    }else{
      query.startDate = that.data.startDate + " 00:00"
    }
    // if (that.data.endDate == "结束时间") {
    if (that.data.endDate == wx.T.get("jieshushijian")) {
      query.endDate = ""
    }else {
  ...
  onLoad: function (options) {
    wx.setNavigationBarTitle({ title: wx.T.get("liulangquanchuli") })
    this.setData({
      startDate: wx.T.get("kaishishijian"),
      endDate: wx.T.get("jieshushijian"),
    })
  ...
  toStrayDogPage(e) {
    ...
    wx.navigateTo({
      url: '/pages/admin/home/strayDog/info/info?strayDog_index=' + JSON.stringify(strayDog_index),
    })
  },
  ...
  toStrayDogAddPage() {
    wx.navigateTo({
      url: '/pages/admin/home/strayDog/add/add',
    })
  }
})