...
Page({
  ...
  formSubmit: function (e) {
    ...
    //表单提交
    var url = config.url.BASE_URL + '/activityInfo/save';
    ...
    http.httpPost(url, token, activityInfo, function (res) {
      ...
      if (res.code == 0) {
        ...
        setTimeout(function () {
          wx.navigateBack()
          wx.redirectTo({
            url: '/pages/admin/home/publicityActivity/publicityActivity',
          })
        }, 1500)
      }
    })
  ...
  onLoad: function (options) {
    wx.setNavigationBarTitle({ title: wx.T.get("xinzengxuanchuanhuodong") })
  ...
  initValidate: function () {
    ...
    var   messages = {
      activityAddress: {
        required: wx.T.get("qingtianxiedidian"),
      },
      activitySubject: {
        required: wx.T.get("qingtianxiezhuti"),
      },
      amount: {
        required: wx.T.get("qingtianxiexuanchuanppingshuliang"),
        digits: wx.T.get("xuanchuanpingchuliangbuzhenfgque"),
      },
      activityContent: {
        required: wx.T.get("qingtianxieneirong"),
      }
