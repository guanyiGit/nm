...
Page({
  ...
  formSubmit(e){
    ...
    this.getContrailByDeviceNoAndDate(deviceNo, date);
  },
  /**
   * 根据溯源id查询犬只
   */
  searchDog(e) {
    ...
    http.httpGet(url, token, params, function(res) {
      ...
      if (dogInfos != null && dogInfos.length>0){
        for (var index in dogInfos){
          if (dogInfos[index].dogRefDeviceRecordMax == null || dogInfos[index].dogRefDeviceRecordMax==''){
            dogInfos.splice(index,1)
          }
        }
      }
  ...
  * 获取犬只轨迹
  */
  getContrailByDeviceNoAndDate(deviceNo, date) {
    var that = this;
    var url = config.url.BASE_URL + "/position/getContrailByDeviceNoAndDate";
    var token = app.globalData.token;
    var params = {
      deviceNo: deviceNo,
      date: date
  ...
  markerClick(e) {
    ...
    wx.navigateTo({
      url: '/pages/home/petInfo/petInfo?deviceNo=' + marker.title,
    })
   ...
   * 获取所有犬只定位
   */
  getDogLocation(userId) {
    var that = this;
    var url = config.url.BASE_URL + "/position/getdogPosition/" + userId;
    var token = app.globalData.token;
    var params = {};
    http.httpGet(url, token, params, function(res) {
      ...
      if (!res || !res.data || res.data.length == 0) {
        ...
        for (var index in res.data) {
          ...
          markers.push({
            ...
            width: 30,
            height: 30,
            title: res.data[index].dogRefDeviceRecordMax.deviceNo,
            callout: {
