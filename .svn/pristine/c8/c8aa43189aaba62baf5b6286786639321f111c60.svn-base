...
Page({
  ...
  formSubmit: function(e) {
    ...
    if (that.data.deviceNo) {
      antiepidemic.deviceNo = that.data.deviceNo
    }
    ...
    //根据溯源号查狗Id
    var url = config.url.BASE_URL + "/dogCancel/findDogIdByDeviceNo";
    var token = app.globalData.token;
    var params = {
      deviceNo: antiepidemic.deviceNo
    }
    http.httpPost(url, token, params, function(res) {
      var dogId = res.data;
      var url = config.url.BASE_URL + '/antiepidemic/save';
      antiepidemic.antiepidemicDate = that.data.antiepidemicDate;
      antiepidemic.dogId = dogId;
