...
public class HttpClientUtil {
     ...
     * @param isCloseHttpClien 执行请求结束后是否关闭HttpClient客户端实例
     * @param headers          (可设置[referer:连接地址]和[Cookie：cookie信息]) 请求头信息
     * @param paramsObj        (可是是String类型和Map类型) 请求参数
     ...
     * @param isCloseHttpClien 执行请求结束后是否关闭HttpClient客户端实例
     * @param headers          (可设置[referer:连接地址]和[Cookie：cookie信息]) 请求头信息
     * @param paramsObj        (可是是String类型和Map<String,String>类型) 请求参数
     * @return HttpResult(状态码 ， 结果)
